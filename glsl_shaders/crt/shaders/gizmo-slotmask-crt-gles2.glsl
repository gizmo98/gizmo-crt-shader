...
vec4 textureAABlur(in vec2 uv){
    ...
    COMPAT_PRECISION vec2 precalc_scale = max(floor(OutputSize.xy / InputSize.xy), vec2(1.0, 1.0));  
    COMPAT_PRECISION vec2 region_range = 0.5 - 0.5 / precalc_scale; 
    COMPAT_PRECISION vec2 center_dist = fuv - 0.5;  
    COMPAT_PRECISION vec2 f = (center_dist - clamp(center_dist, -region_range, region_range)) * precalc_scale + 0.5;  
    
    if (HORIZONTAL_BLUR == 1.0){
        COMPAT_PRECISION vec2 uv1 = vec2(uv + vec2(f)) / TextureSize.xy;
        COMPAT_PRECISION vec2 uv2 = vec2(uv + vec2(f.x - BLUR_OFFSET,f.y)) / TextureSize.xy;
        COMPAT_PRECISION vec4 col1 = COMPAT_TEXTURE( Texture, uv1 );
        COMPAT_PRECISION vec4 col2 = COMPAT_TEXTURE( Texture, uv2 );
        COMPAT_PRECISION vec4 col = (col1 + col2) / vec4(2.0);
        if (VERTICAL_BLUR == 1.0){
            COMPAT_PRECISION vec2 uv3 = vec2(uv + vec2(f.x,f.y -BLUR_OFFSET)) / TextureSize.xy;
            COMPAT_PRECISION vec2 uv4 = vec2(uv + vec2(f.x - BLUR_OFFSET,f.y -BLUR_OFFSET)) / TextureSize.xy;
            COMPAT_PRECISION vec4 col3 = COMPAT_TEXTURE( Texture, uv3 );
            COMPAT_PRECISION vec4 col4 = COMPAT_TEXTURE( Texture, uv4 );
